...
class UnVerifiedManager(models.Manager):
    def get_queryset(self):
        return super(UnVerifiedManager, self).get_queryset()\
...
class Category(models.Model):
    ...
    def questions_count_category() -> Dict[str, int]:
        ...
        for category in categories:
            question_count_category_dict.setdefault(category.name, category.questions.count())
        return question_count_category_dict


class Question(models.Model):
    question = models.TextField()
    difficulty = models.CharField(choices=DIFFICULTY_CHOICES, max_length=100, db_index=True)
    ...
    related_name='questions_verified', blank=True)
    image = models.ImageField(blank=True, upload_to='quiz/')
    category = models.ForeignKey(Category, 
    on_delete=models.SET(get_sentinel_category), related_name='questions')
    ...
    def last_verified() -> datetime.datetime:
        return Question.objects.filter(is_verified=True).order_by(
        '-date_verified').first().date_verified
...
class InCorrectAnswer(models.Model):
    ...
    def clean(self):
        ...
        if self.question.type == 'True / False' and \
        self.question.incorrect_answers.count() >= 1:
            if self.id is None: #ensures that editing old question don't raise this error
                raise ValidationError("True/False questions can have only one \
                incorrect answer")
